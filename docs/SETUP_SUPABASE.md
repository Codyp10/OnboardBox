...
This cloud agent cannot access a desktop-only Supabase MCP connection. Provide the values below as **Cursor Cloud Agent secrets** (or `.env.local` for local runs), then the migration can be applied and demo mode turned off.

## Required secrets (minimum to leave demo mode)
...
1. Create (or open) the OnboardBox Supabase project.
2. Run the SQL migration: `supabase/migrations/20260810000000_init.sql`  
   (SQL Editor → paste/run, or `supabase db push` if the CLI is linked).
